...
classDiagram
    ...
    class ST_Twist {
        <<struct>>
        +LREAL linear_x, linear_y, linear_z
        +LREAL angular_x, angular_y, angular_z
    }
    class ST_MoveAction {
        <<struct>>
        +BOOL execute_goal
    ...
    %% Implementierende Funktionsbausteine
    class FB_Status {
        <<FunctionBlock>>
        +ST_JointStates joint_states
        +ST_TcpPose tcp_pose
        +M_Watchdog()
    }
    class FB_Command {
        <<FunctionBlock>>
        +ST_Twist delta_twist
        +M_Watchdog()
    }
    class FB_Action {
        <<FunctionBlock>>
        +ST_MoveAction move_action
        +M_Watchdog()
    }

    %% Container Funktionsbaustein
    class FB_Robot {
        <<FunctionBlock>>
        +FB_Status Status
        +FB_Command Command
    ...
    class Symbolgruppe_OPC_UA {
        <<OPC_UA_Server>>
        +PRG_MAIN.MyCobot280
    }

    %% Beziehungen (Relations)
    I_Status <|.. FB_Status : implements
    I_Command <|.. FB_Command : implements
    I_Action <|.. FB_Action : implements

    FB_Status o-- ST_JointStates : uses
    FB_Status o-- ST_TcpPose : uses
    FB_Command o-- ST_Twist : uses
    FB_Action o-- ST_MoveAction : uses

    FB_Robot *-- FB_Status : composes
    FB_Robot *-- FB_Command : composes
    FB_Robot *-- FB_Action : composes

    PRG_MAIN *-- FB_Robot : instantiates

    Symbolgruppe_OPC_UA ..> PRG_MAIN : exposes